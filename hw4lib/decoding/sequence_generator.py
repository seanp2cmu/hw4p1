...
class SequenceGenerator:
    ...
    def generate_beam(
        ...
        # Start with a single beam per batch item
        beam_seqs   = x.unsqueeze(1)                                              # (B, 1, S)
        beam_scores = torch.zeros(batch_size, 1, device=device)                  # (B, 1)
        finished    = torch.zeros(batch_size, 1, dtype=torch.bool, device=device) # (B, 1)

        for _ in range(self.max_length - init_len):
            if finished.all():
                break

            current_bw  = beam_seqs.size(1)
            current_len = beam_seqs.size(2)

            # Score each beam independently to preserve batch-tree correspondence
            # score_fn expects (B, S) and uses trees[b] for batch item b
            beam_logits = []
            for bw_idx in range(current_bw):
                seq_bw = beam_seqs[:, bw_idx, :]           # (B, S)
                logits_bw = self.score_fn(seq_bw)           # (B, V)
                logits_bw = self._apply_repeat_penalty(logits_bw, seq_bw, repeat_penalty)
                logits_bw = logits_bw / temperature
                beam_logits.append(logits_bw)
            # Stack: (B, current_bw, V)
            all_logits = torch.stack(beam_logits, dim=1)
            vocab_size = all_logits.size(-1)
            log_probs  = torch.log_softmax(all_logits, dim=-1)

            # Freeze finished beams: only EOS is valid (adds 0 to score)
            if finished.any():
                ...
                log_probs = torch.where(fin_mask, eos_only.expand_as(log_probs), log_probs)

            # Combined scores and flatten over beams & vocab: (B, current_bw * V)
            combined = (beam_scores.unsqueeze(-1) + log_probs).view(batch_size, current_bw * vocab_size)

            k            = min(beam_width, combined.size(-1))
            top_scores, top_flat = combined.topk(k, dim=-1)  # (B, k)

            parent_beams = top_flat // vocab_size  # (B, k)
            next_tokens  = top_flat % vocab_size   # (B, k)

            # Assemble new sequences from chosen parent beams
            parent_exp = parent_beams.unsqueeze(-1).expand(batch_size, k, current_len)
            beam_seqs  = torch.cat([beam_seqs.gather(1, parent_exp), next_tokens.unsqueeze(-1)], dim=-1)

            # Update scores and finished flags
            beam_scores = top_scores
            old_fin     = finished.gather(1, parent_beams)
            finished    = old_fin | (next_tokens == self.tokenizer.eos_id)

        # Sort beams by score descending
        sorted_scores, sort_idx = beam_scores.sort(dim=-1, descending=True)
        sort_idx_exp = sort_idx.unsqueeze(-1).expand(batch_size, beam_width, beam_seqs.size(-1))
        sorted_seqs  = beam_seqs.gather(1, sort_idx_exp)
