...
class MultiHeadAttention:
    ...
    def _merge_masks(self, key_padding_mask, attn_mask):
        ...
        key_mask = key_padding_mask[:, np.newaxis, np.newaxis, :] if key_padding_mask is not None else None
        attention_mask = attn_mask[np.newaxis, np.newaxis, :, :] if attn_mask is not None else None
    ...
    def _split_heads(self, x):
        ...
        x = x.reshape(N, seq, self.num_heads, d_k)
